...
> **REGENERATED 2026-08-21 from the clean re-run of 2026-08-18** (`validation_2026-08/phase5/run_goal1/`,
> commit `0c235f4`, 110 runs in 64.0 min). Every number and figure below is recomputed from that run's
> `results.csv`; the July version of this document reported 100/110 trustworthy and its figures were
> rendered from pre-A-0 data. Headline changes: trustworthy 100 → **93**, median target error 0.082 →
> **0.098**, success rate 42 % → **45 %**, soft-band frontier −0.82 → **−0.78**.
...
- **ν grid (11):** `[-0.9, -0.7, -0.5, -0.3, -0.15, 0.0, 0.1, 0.2, 0.3, 0.4, 0.45]`.
  ...
  > below**: the reported max of **+0.45** at `f=0` (and at `f=0.1`) is *the top grid point* —
  ...
  > is a genuine measurement (grid runs to −0.9, designer falls short at −0.78). Probed properly by
...
|------|-----|------------------------|----------|
| soft   | 0.00 | **[−0.78, +0.45 ⚠]** | +0.05 |
| large  | 0.10 | [−0.18, +0.45 ⚠] | +0.02 |
| medium | 0.50 | [+0.00, +0.40] | +0.20 |
| small  | 0.90 | [+0.16, +0.34] | +0.24 |
| none   | 0.99 | **[+0.20, +0.33]** | +0.28 |

> ⚠ **= CENSORED BY THE GRID, not a measured frontier.** +0.45 is the top grid point, so the `soft`
> and `large` maxima are not properties of the networks — nothing above +0.45 was attempted. The
> `medium`/`small`/`none` maxima (+0.40, +0.34, +0.33) ARE genuine: they sit well inside the grid, so
...
> | large | 0.10 | +0.45 ⚠ | **+0.63** (genuine — targets 0.80/0.90/0.95 all saturate there, gap 0.0000) |
>
...
At `f = 0` the designer reaches deep auxetic behaviour (ν ≈ −0.78) *and* the stiff positive end
(+0.45). Clamping the bonds toward uniform (`f = 0.99`) collapses the whole reachable window to
≈ [+0.20, +0.33], i.e. it hugs the isotropic triangular-lattice ν = 1/3. **Auxeticity requires
...
|------|-----|---------|--------|---------------|
| soft   | 0.00 | 0.379 | 0.009 | **0.008** |
| large  | 0.10 | 0.457 | 0.229 | **0.206** |
| medium | 0.50 | 0.417 | 0.346 | **0.324** |
| small  | 0.90 | 0.405 | 0.395 | **0.376** |
| none   | 0.99 | 0.392 | 0.392 | **0.374** |

Reading this honestly:

- **k is the dominant lever, and it works spectacularly where it is allowed.** In the soft band the
  k-designer alone cuts the error 0.379 → 0.009 (~42×); positions then trim it to **0.008**.
- **Positions help secondarily, most in the mid-contrast bands.** The k→(k+pos) reduction is largest
  in `large` (0.229 → 0.206) and `medium` (0.346 → 0.324) — where k has done what it can but the
...
- **In the near-uniform bands neither lever moves ν much — the response is locked.** At `small`
  (0.405 → 0.395 → 0.376) and `none` (0.392 → 0.392 → 0.374) both k and positions are nearly inert:
...
- **93 / 110 runs trustworthy** (solver-vs-sim gap < 0.05). The 17 dropped runs are dominated by the
  hard corners — extreme ν, or auxetic/soft-edged near-mechanisms where the independent sim and
  solver diverge (worst: tiling `large` ν*=−0.30 gap **1.42**, tiling `medium` ν*=−0.70 gap **1.42**,
  tiling `small` ν*=+0.40 gap 0.46). Dropping them is the point of the honesty gate. Note the drops
  are **not** confined to the extremes: the `large` band alone loses 6 runs including ν* = +0.10 and
  +0.20, so read the band frontiers (§3a) as measured over a thinned sample there.
- **Median target error (k+pos) = 0.098; success rate |err| < 0.05 = 45 %.**
- **Why the success rate is "only" 45 % — and why that is the correct outcome.** The ν grid was
  ...
  are physically accessible, the error is 0.008 and essentially every point is a hit. The 45 % is
...
|-------|-----|---------|-----------|--------------|-----|
| tiling  | 4  | 0.079 | **0.026** | 0.000 | 0.045 |
| auxetic | 6  | 0.283 | **0.089** | 0.001 | 0.156 |
| foam    | 40 | 0.444 | 0.251 | 0.044 | 0.362 |
| bravais | 31 | 0.437 | 0.322 | 0.155 | 0.364 |
| flipped | 12 | 0.391 | 0.261 | 0.294 | 0.216 |
