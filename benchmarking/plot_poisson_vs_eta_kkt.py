...
if os.path.exists(CACHE):
    ...
    for i_eta, eta in enumerate(ETA_VALUES):
        ...
        for trial in range(N_TRIALS):
            ...
            row_old.append(nu_o)
            row_new.append(nu_n)
            print(f"  η={eta:.2f}  trial={trial:2d}  old={nu_o:+.5f}  new={nu_n:+.5f}  Δ={nu_n-nu_o:+.5f}",
                  flush=True)
        print()
    ...
    old_arr = np.array(nu_old_all)   # (11, 10)
...
# ── Plot ──────────────────────────────────────────────────────────────────────
# Clip all values to [-1.5, 0.5] so unstable high-eta outliers don't crush scale
YMIN, YMAX = -1.0, 0.45
old_clip = np.clip(old_arr, YMIN, YMAX)
new_clip = np.clip(new_arr, YMIN, YMAX)

# Stability threshold: beyond this eta meshes are often degenerate
ETA_STABLE = 0.35

fig, axes = plt.subplots(1, 2, figsize=(14, 5), sharey=True)
jitter = 0.005

# --- Left: raw scatter (clipped) ---
ax = axes[0]
...
# --- Right: median ± IQR (robust against outliers) ---
...
out_path = os.path.join(os.path.dirname(__file__), 'poisson_vs_eta_kkt.png')
