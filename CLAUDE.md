...
`draw_field`, `plot_directional`, `plot_means_spread`, `plot_overlay_grid`, **`plot_ranges`** (reach intervals per group: pale = full reach, solid = the solver-sim-agreeing sub-range, points filled/hollow by trust — added 2026-08-22), **`plot_learning_curves`** (log-y training curves with the trivial-predictor baseline drawn as a LINE — added 2026-08-27), `save_element`/`save_fig`, `montage`,
