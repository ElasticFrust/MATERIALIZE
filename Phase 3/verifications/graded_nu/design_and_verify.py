...
def main():
    results = {}                    # (topo,N) -> (xc, tgt, sim_nu per strip)
    rep = None
    ...
    np.savez(os.path.join(d, 'results.npz'),
             ...
             sim_nu=[v[2] for v in results.values()])

    fig, ax = plt.subplots(1, 2, figsize=(13.5, 5.8))
    # panel 0: nu(x) target vs simulated, all topo x size
    xc0, tgt0, _ = next(iter(results.values()))
    ax[0].plot(np.linspace(0, 1, NSTRIP), tgt0, 'k--', lw=2, label='prescribed ν(x)')
    for (topo, N), (xc, tgt, snu) in results.items():
        xn = (xc - xc.min()) / (xc.max() - xc.min())
        ax[0].plot(xn, snu, '-', color=C.TOPO_COLORS[topo], marker=C.SIZE_MARKERS[N], ms=5,
                   lw=1.0, alpha=0.8,
                   label=dict(C.TOPOS[i][:2] for i in range(len(C.TOPOS))).get(topo)
                   if N == C.SIZES[0] else None)
    ax[0].axhline(0, color='gray', lw=0.5, ls=':')
    ax[0].set_xlabel('normalised x'); ax[0].set_ylabel('ν (simulated)')
    ax[0].set_title('Prescribed vs simulated ν(x) gradient\n(all topologies × sizes)')
