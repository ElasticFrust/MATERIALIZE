...
- [ ] **NEXT: return to the plan and write the next phases.** Re-hash and verify the
      *original* project plan (`Phase 4/PLAN.md`) in light of the changes since it was
      written — the forward solver was replaced (intrinsic metric solve is now the
      verified default) and the Phase 3/4 implementation was pruned to the core. Then
      write the updated next phases on top of the current, verified solver. This is the
      first thing to do next session.

- [ ] **(Later, alongside the other core-solver items above) Go 3D.** Extend the whole
      pipeline from 2D triangulated networks to 3D (tetrahedralised networks): the
      per-element stiffness, the edge/curvature/volume-weighted constraints, the
      homogenisation, and the sim-vs-solver verification. Same status as the other
      not-soon core-solver TODOs (O(δ²) term, cluster solver, varying reference metric).

- [ ] **(Future) Local mechanical constraints / targeted regional response.** Support
      designing/prescribing specific mechanical responses in specific regions of the
      network (e.g. a stiff/soft/auxetic patch, a prescribed local deformation mode, a
      region with a target local elastic tensor) rather than only a single homogenised
      macroscopic target. Wire the forward solver's per-region response (already
      available as per-triangle W) into the design objective.
