...
| `Phase 5/verifications/test_m2_head.py` + `Phase 5/m2/train_v2.py` | Is the v2 head expressive, equivariant and SPD — and does it LEARN? | `Phase 5/results/m2_s1/M2_S1.md` — **parametrisation verified 7/7 BEFORE training** (expressive to 3.52e-16 at `max|W|` = 2.43; the plan’s earlier affine-only ansatz would fail this). Training: within-family **0.4613** vs leave-one-family-out **0.5343** MAE/std, gap +16 %; SPD violations 0 by construction. **But on a held-out family it does NOT beat the trivial "predict your own bulk" baseline (0.5144)**, and it is UNDERFIT — still improving at epoch 149. Not yet comparable to §1’s ν/E-against-sim criteria, so S1 is neither passed nor failed |
| `Phase 5/verifications/m2_s1_crystals.py` | Can a SHALLOW GNN learn the k=1 Bravais crystals, where W=0 and C(s)=A(s) is a closed form in the triangle’s own three edges? | `Phase 5/results/m2_s1_crystals/M2_S1_CRYSTALS.md` — **yes: 0.00107 MAE/std with NO message passing (0 layers), 0.00025 with one.** Depth HURTS on a local target (2 layers 13× worse). 792 crystals = 792 samples, ONE distinct C(s) each. Also caught the second unlearnable-scale bug (target in internal units, per-crystal factor 18–220) |
...
| `Phase 5/verifications/test_m2_constraints.py` | Are the solver's three CONSTRAINT operators actually in the model, and are the weights the solver's own? | `Phase 5/results/m2_s1/M2_RESIDUAL_AND_CONSTRAINTS.md` §3–4 — **`J_edge` was already there by accident** (`triangle_adjacency` *is* the shared-bond pairing; weights exact at 0.000e+00 **once contracted with `diag(1,2,1)`**, and a raw comparison shows a spurious 0.5 because the factor 2 lives in the metric). **`C_curv` was entirely absent** → new `StarMP`, kernel 0.000e+00 vs the solver, operator 4.4e-16. `M_S` → `GlobalMS`. Dimension check `3N − rank = 2·nodes − 2` **exact on 3 meshes** (116/116, 230/230, 30/30), stacked operator rank-deficient by 1 = the B-1 redundancy seen independently |
| `Phase 5/verifications/m2_error_strata.py` | WHERE does the surrogate's error live — is the headline average moving for the reason being tested? | `Phase 5/results/m2_s1/M2_RESIDUAL_AND_CONSTRAINTS.md` §7/§7b — error is controlled by **k-CONTRAST, ~15× monotone** (0.0193 → 0.2880), NOT by the `k`-field's correlation length, which works the opposite way. The **‖W‖ split is the training-domain boundary** (`--w_max_cut 10`): 1190 in-domain networks at MAE(ν) **0.0452** vs 110 never-trained-on at **0.2190**, the latter carrying 22 % of the tensor error and **31 %** of MAE(ν) — so the headline is a mixture, and an earlier "roughly two thirds" attribution was wrong. Also found: the residual head is **NOT** exact where `W = 0` after training (254 networks, lowest tensor error of any bin at 0.0405, but MAE(ν) 0.0528 — later repaired to 0.0205 by `M_S` alone) |
| *(ad-hoc, recorded in the results doc)* | Are the solver LABELS trustworthy at high ‖W‖ — i.e. is the `--w_max_cut` tail learnable at all? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §7b — **its premise is REFUTED.** On the 6 020 samples where `build_dataset` actually computes `sim_gap` (`structure == 'dilution'`, the case the solver was *expected* to be fragile in), the solver–sim gap **does not degrade with ‖W‖, it improves**: max gap 4.97e-02 at ‖W‖ 1–3 falling to **7.04e-03** above 100, with **zero** samples over the 0.05 flag in any bin. ⚠ `sim_gap` is `0.0`/`not_checked` for every NON-dilution sample (`build_dataset.py:527`) — a first pass read those placeholders as measurements |
| `Phase 5/verifications/m2_fresh_holdout.py` | How does a TRAINED surrogate do on meshes it has never seen? — without retraining under a new split | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9 — **a frozen model + a seeded generator gives honest unseen test data for a 25-min dataset build instead of a 2.4-day retrain** (the split must be fixed before TRAINING; the MEASUREMENT need not be). Generalisation to unseen meshes **+9.7 %** (train 0.1197 → fresh 0.1313). Gates mesh disjointness BEFORE measuring and uses the ORIGINAL training σ |
| `Phase 5/verifications/m2_error_cancellation.py` | Do per-triangle errors CANCEL in the bulk mean, or are they systematically correlated? — i.e. is a bulk loss term treating the real mechanism? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9c — `R = mean|err|/|mean err|` tracks **√N at a CONSTANT 0.435** in every family ⇒ errors independent over ~5.3 triangles, **NO systematic bias**. So `cells` fails purely from weak √N averaging on a 16-triangle mesh, and a bulk term suppresses no bias — it acts only by per-graph re-weighting. ⚠ `n_corr` is a COUNT, not a radius: an earlier draft compared it to a cluster RADIUS of 4–6, a units error |
| `Phase 5/verifications/m2_validation_plots.py` | The figure set: architecture ladder, and model/solver vs the SIM, aggregate and stratified | `Phase 5/results/m2_s1/m2_*.png` — parity panels carry the SOLVER's own scatter in the same axes, so the model's residual is never confused with the two code paths' own disagreement. ⚠ `cells` is the ONLY family with a non-zero solver-vs-sim floor (MAE(ν) ~0.014) |
| `Phase 5/verifications/test_m2_dataset_seeding.py` | Does `--seed` actually produce a DIFFERENT dataset? (gate, seconds) | commit `ad13ace` — it did not: `--seed` missed `bravais`/`disordered`, so "fresh" builds reused **389 of 603 meshes**. Fixed; **phi must be RATIONAL** under PBC (a continuous draw silently produced ZERO meshes in three families and the first version of this gate PASSED it). Identity at seed 0, so `dataset_v2_s0` stays reproducible |
| *(ad-hoc, recorded in the results doc)* | Is the surrogate over- or under-fitting — more data or more capacity? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §7b — **TRAIN 0.1214 vs VAL 0.0708** — train 71 % worse. RULES OUT overfitting (so more data is not indicated) but does **NOT** establish underfitting: train > val is not the underfit signature (that is train ≈ val), it means train and val are different DISTRIBUTIONS, which leave-one-family-out guarantees. **The decisive test is an OVERFIT PROBE** — fit ~200 networks to convergence; error near zero ⇒ capacity is fine. ⚠ `m2_v3_report`'s own train score is computed on the **UNFILTERED** split, so it includes the 34.6 % of samples `--w_max_cut` removed and the model never saw — **that number (0.1771) is contaminated; do not quote it.** Bounded by: leave-one-family-out makes train/val different distributions, so part of the gap is family difficulty |
| `Phase 5/verifications/m2_probe_vs_trained.py` (+ `Phase 5/m2/train_v3.py --overfit_probe`) | Is the surrogate's high-contrast error a CAPACITY floor or a generalisation gap? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9f + `results/m2_s1/probe_vs_trained.json` — **a FITTING failure.** Both checkpoints on the **same 200 high-contrast samples under ONE `sd`**: trained-on-40 775 **0.3455** vs the probe that memorises only those 200 **0.2213** — all capacity aimed at the hard population buys just −36 % and nowhere near zero, so more data cannot help. **Bulk 0.0615 vs per-triangle 0.2213** localises it to the SPATIAL structure, not the graph mean. L5 stopped on its own rule at ep 442; L10 stalled at 0.5258 and the L8 arm diverged ⇒ depth past 5 is an OPTIMISATION problem. ⚠ Bounded three ways: **1.4:1** params-to-target-scalars is not "free to memorise" (needs ~16 samples for a clean capacity test), convergence rests on the plateau rule and **not** on the decisive LR-RESTART probe, and reach-vs-expressivity is still NOT separated |
| `Phase 5/verifications/m2_probe_vs_trained.py --n 8` | Is the fitting floor a CAPACITY shortage? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9g + `probe_vs_trained_n8.json` — **NO, capacity is REFUTED.** At **39.7:1** params-to-target-scalars and ~9 480 gradient steps (vs 1.4:1 and ~3 100) the floor moves only **0.2213 → 0.1936, −12.5 %**; bulk 0.0571 vs per-triangle 0.1936 puts the deficit in the SPATIAL structure. ⇒ neither more parameters nor more data is the lever, and `--graph_balance` (per-GRAPH re-weighting) does not address a per-TRIANGLE deficit. ⚠ Ended on the **LR FLOOR** with val still creeping, so **expressivity vs optimisation is undecided** — the LR-RESTART probe of `CLAUDE.md` §3 is the outstanding test |
| `Phase 5/m2/train_v3.py --restart_lr <rate> --resume` | Was the flat tail the model finding the bottom, or the schedule no longer letting it walk? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9h — **the SCHEDULE.** Restarting the "converged" 8-sample probe at **2.7e-4** reached **0.1636 from 0.1936, −15.5 %**; at the INITIAL **3e-3** it threw the fit to 0.73 and never re-entered the basin (**instrument failure** — restore a rate the run was still progressing at, not the initial one). ⚠ **Levels of EVERY S1 trained number are therefore pessimistic** (0.2213 → 0.1936 → 0.1636, each "converged"); ranking probably safe since all arms shared the schedule, **but they do NOT reach zero**. ⇒ a representational deficit survives (~0.13-0.15 on 8 memorisable samples at 39.7:1). ⚠ **SCOPE: the 8-sample PROBE only** — on the REAL arm the same test does NOT improve (§9i: best 0.0938 vs 0.0925, **1.4 % worse**, flat over 8 060 steps), so "S1 levels are pessimistic by 20-30 %" is **WITHDRAWN** and the ladder/tier numbers stand as measured. They reconcile because patience is counted in EPOCHS: stop window = **800 steps (probe) vs 19 344 (real arm)**. **Compute the stop window in STEPS before claiming a run stopped early** |

| `Phase 5/m2/train_v3.py --w_max_cut 0 --init_from` (+ `m2_error_strata`, `m2_fresh_holdout`) | Does training on the near-mechanism TAIL, excluded on a premise measured false, reduce the error? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9j — **NO: NEGATIVE on the pre-registered rule.** Tail TENSOR error falls ~20 % (−20.3 % holdout, −21.6 % fresh) and **E improves hugely** (rel_E −68.5 % in-domain, −83.5 % tail), but **tail MAE(ν) WORSENS** (+11.3 %, +9.4 %) and in-domain regresses (+24.4 % on ν). Overall MAE/σ **0.0908 → 0.0920, conserved** ⇒ the error was **REDISTRIBUTED, not reduced**: the model is **representation-limited, not data-limited**, agreeing with §9g (capacity) and the +9.7 % data gap. ⚠ warm-start ≠ from-scratch; and pass `--fresh dataset_fresh_s4321.npz` explicitly — the default shares 389 meshes with training |

| `Phase 5/verifications/m2_conditioning_vs_error.py` | Does the conditioning of the solver's constraint system predict the SURROGATE's error -- i.e. is the deficit an ITERATION-count problem? | `M2_RESIDUAL_AND_CONSTRAINTS.md` §9k — **NO. Hypothesis NOT SUPPORTED.** On 400 `bravais` networks `cond(G)` explains a **7.33×** spread in median abs(dnu) against **max abs(W)'s 8.96×** (rank corr +0.291 vs +0.362), and the two are collinear (**Spearman +0.772**) — `cond(G)` is largely max abs(W) in disguise. Stratified both ways it is non-monotone (1.86/0.93/4.30 and 1.07/1.46/5.62): both are threshold indicators of near-mechanism that bite only in the top bin. ⇒ does NOT justify the recurrent architecture; the iteration idea is UNSUPPORTED, not refuted, and its only direct test is the weight-tied model itself. Captures G by monkey-patching `lstsq` — protected core untouched |
